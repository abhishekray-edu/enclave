...
highlight.js for rendering. A local [Ollama](https://ollama.com) engine is also available for
native-speed, larger models.

**In-browser inference (WebGPU via WebLLM).** The model (default **Qwen3 4B**, 4-bit) runs in
the browser with GPU acceleration. On first use the weights download once (~2.5 GB) into the
browser cache and are reused afterwards. Inference runs at ~80% of native speed on Apple
Silicon. The context window is configurable and defaults to the model build's full window
(40,960 for Qwen3 4B). The KV cache is pre-allocated for the configured window at load time,
so it reserves GPU memory up front regardless of how much is used; lower it on
memory-constrained machines. WebGPU and a CSP allowing `wasm-unsafe-eval` (for the
WebAssembly runtime) are required.
